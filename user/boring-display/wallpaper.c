...
static uint32_t scale_reference(uint32_t value, uint32_t actual,
                                uint32_t reference) {
    return (uint32_t)(((uint64_t)value * (uint64_t)actual) /
                      (uint64_t)reference);
}

static bool background(const struct boring_display_core *core,
                       uint8_t *output, size_t size) {
    const uint32_t glow_width = scale_reference(520U, core->width, 800U);
    const uint32_t glow_center_y =
        scale_reference(365U, core->height, 600U);
    const uint32_t glow_radius_y =
        scale_reference(300U, core->height, 600U);
    uint32_t y;

    for (y = 0U; y < core->height; ++y) {
        uint32_t x;
        const uint32_t vertical_distance =
            y > glow_center_y ? y - glow_center_y : glow_center_y - y;
        for (x = 0U; x < core->width; ++x) {
            ...
            hash ^= y * 0x119de1f3U;
            hash ^= hash >> 16U;
            if ((glow_width != 0U) && (glow_radius_y != 0U) &&
                (x < glow_width) && (vertical_distance < glow_radius_y)) {
                const uint64_t numerator =
                    (uint64_t)(glow_width - x) *
                    (uint64_t)(glow_radius_y - vertical_distance) * 18ULL;
                const uint64_t denominator =
                    (uint64_t)glow_width * (uint64_t)glow_radius_y;
                glow = (uint32_t)(numerator / denominator);
            }
            noise = (hash >> 29U) & 3U;
...
static bool logo(const struct boring_display_core *core,
    ...
    const uint32_t origin_y = scale_reference(529U, core->height, 600U);
    uint32_t x = scale_reference(596U, core->width, 800U);
    size_t index;
...
bool display_wallpaper_compose(const struct boring_display_core *core,
    ...
    if ((core == NULL) || (output == NULL) ||
        ...
        (core->width > UINT32_MAX / BORING_DISPLAY_BYTES_PER_PIXEL) ||
        (core->stride <
         core->width * BORING_DISPLAY_BYTES_PER_PIXEL) ||
