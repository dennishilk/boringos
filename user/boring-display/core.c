...
static uint32_t next_generation(uint32_t generation) {
    if ((generation == 0U) || (generation >= SURFACE_GENERATION_MAX)) { return 1U; }
...
static const struct boring_display_surface_state *surface_lookup(
    const struct boring_display_core *core, uint32_t token) {
    uint32_t low;
    uint32_t generation;
    size_t slot;
    if ((core == NULL) || (token == BORING_DISPLAY_SURFACE_INVALID)) { return NULL; }
    low = token & SURFACE_SLOT_MASK;
    generation = token >> SURFACE_SLOT_BITS;
    if ((low == 0U) || (low > BORING_DISPLAY_SURFACE_MAX) || (generation == 0U)) {
        return NULL;
    }
    slot = (size_t)(low - 1U);
...
static struct boring_display_surface_state *surface_lookup_mutable(
    struct boring_display_core *core, uint32_t token) {
    return (struct boring_display_surface_state *)(uintptr_t)surface_lookup(core, token);
}
...
bool boring_display_core_init(struct boring_display_core *core,
    ...
    minimum_stride = (uint64_t)info->width * BORING_DISPLAY_BYTES_PER_PIXEL;
    if (((uint64_t)info->stride < minimum_stride) ||
...
uint32_t boring_display_validate_create(
    ...
    row_bytes = (uint64_t)request->width * BORING_DISPLAY_BYTES_PER_PIXEL;
    if ((row_bytes > UINT32_MAX) || (request->stride != (uint32_t)row_bytes) ||
        ((uint64_t)request->height > UINT64_MAX / row_bytes)) {
        return BORING_DISPLAY_STATUS_INVALID;
    }
    required = row_bytes * request->height;
...
uint32_t boring_display_surface_add(struct boring_display_core *core,
    ...
    if (core->next_creation_order == 0ULL) { return BORING_DISPLAY_STATUS_INTERNAL; }
    for (index = 0U; index < (size_t)BORING_DISPLAY_SURFACE_MAX; ++index) {
        ...
        if (token == BORING_DISPLAY_SURFACE_INVALID) { return BORING_DISPLAY_STATUS_INTERNAL; }
        ...
        core->next_creation_order = core->next_creation_order == UINT64_MAX ?
...
uint32_t boring_display_surface_destroy(struct boring_display_core *core,
                                        ...
                                        uint8_t **pixels_out) {
    struct boring_display_surface_state *surface = surface_lookup_mutable(core, token);
    if ((surface == NULL) || (buffer_handle_out == NULL) || (pixels_out == NULL)) {
        return BORING_DISPLAY_STATUS_INVALID;
    }
    if (surface->owner_endpoint != owner_endpoint) { return BORING_DISPLAY_STATUS_ACCESS; }
    ...
    *pixels_out = surface->pixels;
    ...
    if (core->live_surfaces != 0U) { --core->live_surfaces; }
    return BORING_DISPLAY_STATUS_OK;
}
...
size_t boring_display_peer_cleanup(struct boring_display_core *core,
    ...
    if ((core == NULL) || (owner_endpoint == 0U) ||
        (handles == NULL) || (pixels == NULL)) { return 0U; }
    for (index = 0U; index < (size_t)BORING_DISPLAY_SURFACE_MAX; ++index) {
        struct boring_display_surface_state *surface = &core->surfaces[index];
        if (!surface->active || (surface->owner_endpoint != owner_endpoint)) { continue; }
...
static uint32_t clamp_axis(uint32_t current, int32_t delta, uint32_t limit) {
    ...
    if (next < 0LL) { return 0U; }
    if ((uint64_t)next >= (uint64_t)limit) { return limit - 1U; }
...
static void output_pixel(uint8_t *output, uint32_t stride,
                         uint32_t x, uint32_t y,
                         uint8_t red, uint8_t green, uint8_t blue) {
    const size_t offset = (size_t)y * stride + (size_t)x * BORING_DISPLAY_BYTES_PER_PIXEL;
    output[offset] = blue;
...
static void compose_surface(const struct boring_display_core *core,
                            ...
                            size_t slot, uint8_t *output) {
    uint32_t origin_x = 80U + (uint32_t)slot * 100U;
    uint32_t origin_y = 80U + (uint32_t)slot * 60U;
    uint32_t y;
    if ((surface == NULL) || !surface->active || (surface->pixels == NULL)) { return; }
    if (origin_x >= core->width) { origin_x = core->width - 1U; }
    if (origin_y >= core->height) { origin_y = core->height - 1U; }
    for (y = 0U; y < surface->height && origin_y + y < core->height; ++y) {
        uint32_t x;
        for (x = 0U; x < surface->width && origin_x + x < core->width; ++x) {
            const size_t source = (size_t)y * surface->stride + (size_t)x * 4U;
            const size_t destination =
                (size_t)(origin_y + y) * core->stride + (size_t)(origin_x + x) * 4U;
...
static bool compose_surfaces(const struct boring_display_core *core, uint8_t *output) {
    ...
    for (rendered = 0U; rendered < (size_t)core->live_surfaces; ++rendered) {
        ...
        size_t index;
        for (index = 0U; index < (size_t)BORING_DISPLAY_SURFACE_MAX; ++index) {
            const struct boring_display_surface_state *surface = &core->surfaces[index];
            if (!surface->active || (surface->creation_order == 0ULL) ||
                (surface->creation_order <= previous_order)) { continue; }
...
void boring_display_compose_cursor(const struct boring_display_core *core, uint8_t *output) {
    ...
    for (row = 0U; row < BORING_DISPLAY_CURSOR_HEIGHT; ++row) {
        ...
        for (column = 0U; column <= row / 2U; ++column) {
            ...
            if ((x >= core->width) || (y >= core->height)) { continue; }
            output_pixel(output, core->stride, x, y,
                         column == 0U ? 255U : 48U, 238U, 245U);
        }
    }
}

static struct boring_display_region cursor_region(const struct boring_display_core *core) {
    struct boring_display_region region = {0U, 0U, 0U, 0U};
    if ((core == NULL) || (core->cursor_x >= core->width) ||
        (core->cursor_y >= core->height)) { return region; }
    region.x = core->cursor_x;
    region.y = core->cursor_y;
    region.width = core->width - region.x;
    if (region.width > BORING_DISPLAY_CURSOR_WIDTH) { region.width = BORING_DISPLAY_CURSOR_WIDTH; }
    region.height = core->height - region.y;
    if (region.height > BORING_DISPLAY_CURSOR_HEIGHT) { region.height = BORING_DISPLAY_CURSOR_HEIGHT; }
    return region;
}
...
static void cursor_underlay_copy(struct boring_display_cursor_damage *damage,
                                 const struct boring_display_core *core,
                                 uint8_t *output,
    ...
    for (row = 0U; row < region->height; ++row) {
        ...
        for (column = 0U; column < region->width; ++column) {
            const size_t frame_offset =
                (size_t)(region->y + row) * core->stride +
                (size_t)(region->x + column) * BORING_DISPLAY_BYTES_PER_PIXEL;
            const size_t saved_offset =
                ((size_t)row * BORING_DISPLAY_CURSOR_WIDTH + column) *
                BORING_DISPLAY_BYTES_PER_PIXEL;
            size_t byte;
            for (byte = 0U; byte < BORING_DISPLAY_BYTES_PER_PIXEL; ++byte) {
                if (restore) { output[frame_offset + byte] = damage->saved[saved_offset + byte]; }
                else { damage->saved[saved_offset + byte] = output[frame_offset + byte]; }
            }
        }
    }
}
...
static uint64_t region_pixels(const struct boring_display_region *region) {
    return (uint64_t)region->width * region->height;
}

static void counter_add(uint64_t *counter, uint64_t value) {
    if (value > UINT64_MAX - *counter) { *counter = UINT64_MAX; }
    else { *counter += value; }
}

static uint64_t cursor_drawn_pixels(const struct boring_display_core *core) {
    uint64_t count = 0ULL;
    uint32_t row;
    for (row = 0U; row < BORING_DISPLAY_CURSOR_HEIGHT; ++row) {
        uint32_t column;
        for (column = 0U; column <= row / 2U; ++column) {
            if ((core->cursor_x + column < core->width) &&
                (core->cursor_y + row < core->height)) { ++count; }
        }
    }
    return count;
}

void boring_display_cursor_damage_init(struct boring_display_cursor_damage *damage) {
    size_t index;
    if (damage == NULL) { return; }
    for (index = 0U; index < sizeof(damage->saved); ++index) { damage->saved[index] = 0U; }
...
bool boring_display_cursor_damage_reset(struct boring_display_cursor_damage *damage,
                                        const struct boring_display_core *core,
                                        uint8_t *output, size_t output_size) {
    struct boring_display_region region;
    if (!cursor_damage_arguments_valid(damage, core, output, output_size)) { return false; }
...
bool boring_display_cursor_damage_restore(struct boring_display_cursor_damage *damage,
                                          const struct boring_display_core *core,
                                          uint8_t *output, size_t output_size) {
    struct boring_display_region expected;
    if (!cursor_damage_arguments_valid(damage, core, output, output_size) || !damage->ready) {
        return false;
    }
    ...
    if ((damage->saved_region.x != expected.x) ||
        ...
        (damage->saved_region.height != expected.height)) { return false; }
...
bool boring_display_cursor_damage_move(struct boring_display_cursor_damage *damage,
    ...
    if ((old_region == NULL) || (new_region == NULL)) { return false; }
    ...
    *new_region = (struct boring_display_region){0U, 0U, 0U, 0U};
    ...
    if ((core->cursor_x == old_x) && (core->cursor_y == old_y)) { return true; }
...
bool boring_display_compose(const struct boring_display_core *core,
    ...
    if (!compose_surfaces(core, output)) { return false; }
    boring_display_compose_cursor(core, output);
    return true;
}