...
#define TERMINAL_KEY_BYTES_MAX 4U
...
static size_t key_bytes(uint32_t code, uint32_t modifiers,
                        char output[TERMINAL_KEY_BYTES_MAX]) {
...
static void handle_key(const struct boring_wm_message *message) {
    char bytes[TERMINAL_KEY_BYTES_MAX];
    size_t length;
    if ((message == NULL) || (message->x != (uint32_t)BORING_KEY_DOWN_VALUE)) {
        return;
    }
    length = key_bytes(message->surface, message->y, bytes);
    if (length != 0U) {
