...
# Input core: explicit owner-wait state for the blocking read contract.
replace(
    "kernel/include/boring/input.h",
    "    bool owned;\n    bool initialized;\n",
    "    bool owned;\n    bool waiting;\n    bool initialized;\n",
)
replace(
...
# Keep the translated Set-1 config captured while both ports were disabled;
# do not ask the controller for a config byte after device scanning is live.
replace(
...
# Syscalls 22-24, using existing process and validated user-copy machinery.
replace(
...
replace(
    ...
    r'''static int syscall_input_error(enum boring_input_result result) {
    switch (result) {
...
# Persistent-root input bring-up is after the M30 dashboard and non-fatal.
replace(
...
replace(
    ...
    r'''static uint8_t boringfs_read_buffer[VFS_IO_MAX];
...
replace(
    "kernel/core/boringfs_ro_test.c",
    "#endif\n\n    if (!process_create(&init_process)",
    "#endif\n\n#if BORING_TEST_MODE == 15\n"
...
# BoringFS fixture: preserve the v0 format and use the final fixed 16-block
# program slot for /bin/input-test.
replace(
...
# Standalone input-test: force the data segment and exercise negative ABI cases.
replace(
    "user/input-test/main.c",
...
replace(
    "Makefile",
    ".PHONY: all kernel user-elf user-runtime user-console user-init user-shell user-boringfetch user-cat elf-audit runtime-audit console-audit init-audit shell-audit boringfetch-audit cat-audit shell-host-test fd-host-test framebuffer-host-test boringfs-host-test",
    ".PHONY: all kernel user-elf user-runtime user-console user-init user-shell user-boringfetch user-cat user-input-test elf-audit runtime-audit console-audit init-audit shell-audit boringfetch-audit cat-audit input-test-audit shell-host-test fd-host-test framebuffer-host-test input-host-test boringfs-host-test",
)
replace(
    "Makefile",
    "user-cat: $(CAT_ELF)\n\nelf-audit:",
...
replace(
    "Makefile",
    "\t$(BORINGFS_FIXTURE) $(BUILD_DIR)/boringos-qemu-x86_64/boringos-root.img valid $(BORINGFETCH_ELF) $(CAT_ELF)\n",
...
# The aggregate test target exists exactly once and retains all old gates.
replace(
    "Makefile",
...
# Pure host waiter checks and proper prototypes for the CPU stubs.
replace(
    "tests/input-host-test.c",
    "#include <boring/input.h>\n",
...
print("M31 integration edits applied")
