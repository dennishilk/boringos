...
#define M39_EXTRA_PROGRAMS 6U
#define M39_TERMINAL_OBJECT_ID 13U
#define M39_SHELL_OBJECT_ID 14U
#define M39_DISPLAY_OBJECT_ID 15U
#define M39_WM_OBJECT_ID 16U
#define M39_BLOCK_OWNER_CAPACITY 512U

struct m39_extra_program {
    const char *name;
    uint8_t *bytes;
    size_t size;
    uint32_t object_id;
...
static bool build_bundle(
    ...
    struct m39_extra_program extras[M39_EXTRA_PROGRAMS],
    uint8_t **volume_out, size_t *volume_size_out) {
    uint32_t block_owner[M39_BLOCK_OWNER_CAPACITY];
    ...
    if ((volume_out == NULL) || (volume_size_out == NULL) ||
        (blocks > M39_BLOCK_OWNER_CAPACITY)) {
        return false;
    }
    *volume_out = NULL;
    *volume_size_out = 0U;
    ...
    for (index = 0U; index < M39_EXTRA_PROGRAMS; ++index) {
        uint32_t block;
    ...
    if (!make_object(volume, volume_size, &super, 11U, 1U,
                     ...
                     (1ULL + (uint64_t)M39_EXTRA_PROGRAMS) *
...
int main(int argc, char **argv) {
    ...
    struct m39_extra_program extras[M39_EXTRA_PROGRAMS] = {
        {"boring-terminal", NULL, 0U, M39_TERMINAL_OBJECT_ID, 0U, 0U},
        {"boring-shell", NULL, 0U, M39_SHELL_OBJECT_ID, 0U, 0U},
        {"boring-display", NULL, 0U, M39_DISPLAY_OBJECT_ID, 0U, 0U},
        {"boringwm", NULL, 0U, M39_WM_OBJECT_ID, 0U, 0U},
    ...
    if (argc != 9) {
        (void)fprintf(stderr,
            "usage: %s output boringfetch boring-terminal boring-shell boring-display boringwm boring-edit cat\n",
            argv[0]);
        return 2;
    }
    ...
    minimum = cursor;
    if ((minimum > M39_BLOCK_OWNER_CAPACITY) ||
    ...
    (void)printf("M39 BoringFS measured geometry: %u blocks\n", minimum);
    (void)printf("M39 lower bound: %u blocks rejected\n", minimum - 1U);
    (void)printf("M39 boringfetch: %zu bytes, %u blocks\n",
                 boringfetch_size, boringfetch_blocks);
    for (index = 0U; index < M39_EXTRA_PROGRAMS; ++index) {
        (void)printf("M39 %s: %zu bytes, %u blocks\n",
    ...
    (void)printf("M39 writable geometry: %u blocks (32 free document blocks)\n", minimum + 32U);
    status = write_image(argv[1], volume, volume_size);

cleanup:
    free(volume);
    for (index = M39_EXTRA_PROGRAMS; index > 0U; --index) {
