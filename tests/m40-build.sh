...
make TEST_MODE=m36-desktop boringfsck user-boringfetch user-shell \
    user-boring-terminal user-boringwm user-boring-edit user-cat user-boring-files
...
BUILDER=build/boringfs-m40-bundle
...
"${BUILDER}" "${out}/boringos-root.img" build/user/boringfetch.elf \
    ...
    build/user/boring-edit.elf build/user/cat.elf build/user/boring-files.elf > "${out}/geometry.txt"
build/boringfsck "${out}/boringos-root.img" > "${out}/boringfsck.txt"
grep -Fqx 'Status: VALID' "${out}/boringfsck.txt"
for program in boringfetch boring-terminal boring-shell boring-display boringwm boring-edit cat boring-files; do
...
RAW_STDOUT=build/m40-iso-audit-xorriso.stdout
RAW_STDERR=build/m40-iso-audit-xorriso.stderr
...
AUDIT_DIR=build/m40-iso-audit
rm -rf "${AUDIT_DIR}"
mkdir -p "${AUDIT_DIR}"
xorriso -osirrox on -indev build/boringos.iso -extract /boot/user "${AUDIT_DIR}" \
    >build/m40-iso-audit-extract.stdout 2>build/m40-iso-audit-extract.stderr
FILES=$(find "${AUDIT_DIR}" -type f -printf '/boot/user/%P\n' | LC_ALL=C sort -u)
if [ "${FILES}" != '/boot/user/boring-init.elf' ]; then
    printf '%s\n' 'M40 ISO audit FAILED: expected exactly /boot/user/boring-init.elf.' >&2
    printf '%s\n' "${FILES}" >&2
    exit 1
fi
printf '%s\n' 'M40 ISO audit: exactly one userspace boot module verified.'
sha256sum build/boringos.iso build/user/boring-init-desktop.elf > \
    build/m40-iso-SHA256SUMS
printf '%s\n' 'M40 writable files image and one-module ISO build passed.'
