...
#define M40_EXTRA_PROGRAMS 7U
#define M40_TERMINAL_OBJECT_ID 13U
#define M40_SHELL_OBJECT_ID 14U
#define M40_DISPLAY_OBJECT_ID 15U
#define M40_WM_OBJECT_ID 16U
#define M40_BLOCK_OWNER_CAPACITY 512U

struct m40_extra_program {
    const char *name;
    uint8_t *bytes;
    size_t size;
    uint32_t object_id;
...
static bool build_bundle(
    ...
    struct m40_extra_program extras[M40_EXTRA_PROGRAMS],
    uint8_t **volume_out, size_t *volume_size_out) {
    uint32_t block_owner[M40_BLOCK_OWNER_CAPACITY];
    ...
    if ((volume_out == NULL) || (volume_size_out == NULL) ||
        (blocks > M40_BLOCK_OWNER_CAPACITY)) {
        return false;
    }
    *volume_out = NULL;
    *volume_size_out = 0U;
    ...
    for (index = 0U; index < M40_EXTRA_PROGRAMS; ++index) {
        uint32_t block;
    ...
    if (!make_object(volume, volume_size, &super, 11U, 1U,
                     ...
                     (1ULL + (uint64_t)M40_EXTRA_PROGRAMS) *
...
int main(int argc, char **argv) {
    ...
    struct m40_extra_program extras[M40_EXTRA_PROGRAMS] = {
        {"boring-terminal", NULL, 0U, M40_TERMINAL_OBJECT_ID, 0U, 0U},
        {"boring-shell", NULL, 0U, M40_SHELL_OBJECT_ID, 0U, 0U},
        {"boring-display", NULL, 0U, M40_DISPLAY_OBJECT_ID, 0U, 0U},
        {"boringwm", NULL, 0U, M40_WM_OBJECT_ID, 0U, 0U},
        {"boring-edit", NULL, 0U, 17U, 0U, 0U},
        {"cat", NULL, 0U, 18U, 0U, 0U},
        {"boring-files", NULL, 0U, 19U, 0U, 0U}
    };
    uint8_t *volume = NULL;
    ...
    if (argc != 10) {
        (void)fprintf(stderr,
            "usage: %s output boringfetch boring-terminal boring-shell boring-display boringwm boring-edit cat boring-files\n",
            argv[0]);
        return 2;
    }
    ...
    minimum = cursor;
    if ((minimum > M40_BLOCK_OWNER_CAPACITY) ||
    ...
    (void)printf("M40 BoringFS measured geometry: %u blocks\n", minimum);
    (void)printf("M40 lower bound: %u blocks rejected\n", minimum - 1U);
    (void)printf("M40 boringfetch: %zu bytes, %u blocks\n",
                 boringfetch_size, boringfetch_blocks);
    for (index = 0U; index < M40_EXTRA_PROGRAMS; ++index) {
        (void)printf("M40 %s: %zu bytes, %u blocks\n",
                     extras[index].name, extras[index].size,
                     extras[index].blocks);
    }
    free(volume);
    volume = NULL;
    /* Measured executable lower bound plus 32 writable document blocks. */
    if (!build_bundle(minimum + 32U, boringfetch, boringfetch_size, extras,
                      &volume, &volume_size)) { goto cleanup; }
    (void)printf("M40 writable geometry: %u blocks (32 free document blocks)\n", minimum + 32U);
    status = write_image(argv[1], volume, volume_size);

cleanup:
    free(volume);
    for (index = M40_EXTRA_PROGRAMS; index > 0U; --index) {
