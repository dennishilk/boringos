...
ELF="${ROOT}/build/user/boring-init.elf"

make -C "${ROOT}" user-init
...
if [ "${LOAD_COUNT}" -ne 3 ]; then
    echo "expected exactly 3 boring-init PT_LOAD entries, got ${LOAD_COUNT}" >&2
    exit 1
fi

if printf '%s\n' "${PROGRAMS}" | grep -Eq '^  (INTERP|DYNAMIC|TLS)'; then
    echo 'unexpected PT_INTERP/PT_DYNAMIC/PT_TLS in boring-init' >&2
    exit 1
fi
...
if printf '%s\n' "${PROGRAMS}" | grep '^  LOAD' | grep -Eq ' W E '; then
    echo 'boring-init contains a writable+executable PT_LOAD' >&2
    exit 1
fi
...
if [ "${DATA_FILESZ_DEC}" -ge "${DATA_MEMSZ_DEC}" ]; then
    echo "boring-init data PT_LOAD does not contain real BSS: filesz=${DATA_FILESZ} memsz=${DATA_MEMSZ}" >&2
    exit 1
fi

printf '%s\n' "${SECTIONS}" | grep -Eq '\.bss +NOBITS'
if printf '%s\n' "${SECTIONS}" | grep -Eq '\.(tdata|tbss) '; then
    echo 'unexpected TLS section in boring-init' >&2
    exit 1
fi

if printf '%s\n' "${DYNAMIC}" | grep -Fq '(NEEDED)'; then
    echo 'boring-init has a dynamic library dependency' >&2
    exit 1
fi
...
if [ -n "${UNDEFINED}" ]; then
    echo 'boring-init has unresolved external symbols:' >&2
    printf '%s\n' "${UNDEFINED}" >&2
    exit 1
fi

for symbol in _start boring_main boring_getpid boring_console_write; do
    if ! printf '%s\n' "${SYMBOLS}" | grep -Eq " [Tt] ${symbol}$"; then
        echo "missing required boring-init symbol: ${symbol}" >&2
        exit 1
    fi
done

if printf '%s\n' "${SYMBOLS}" | grep -Eq '(__libc_start_main|__stack_chk_fail|(^| )_init$|(^| )_fini$| printf$| snprintf$| malloc$| free$)'; then
    echo 'unexpected host CRT/libc-style symbol in boring-init' >&2
    exit 1
fi

printf '%s\n' 'boring-init build audit passed.'
printf 'boring-init size: %s bytes\n' "$(wc -c < "${ELF}")"
printf '%s\n' "${TEXT_LINE}"
printf '%s\n' "${RODATA_LINE}"
printf '%s\n' "${DATA_LINE}"
