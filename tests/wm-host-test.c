...
static void policy(void) {
    ...
    check(strcmp(wm_application_path(BORING_KEY_ENTER), "/bin/boring-terminal") == 0, "real terminal path");
    key.code = BORING_KEY_E;
    check(wm_key(&wm, &key) == WM_NO_LAUNCHER && strcmp(wm_application_path(key.code), "/bin/boring-edit") == 0, "real edit shortcut");
    key.code = BORING_KEY_F;
    check(wm_key(&wm, &key) == WM_NO_LAUNCHER && strcmp(wm_application_path(key.code), "/bin/boring-files") == 0, "real files shortcut");
    check(wm_application_path(BORING_KEY_A) == NULL, "no invented launcher");
    key.flags = BORING_INPUT_FLAG_REPEAT;
    check(wm_key(&wm, &key) == WM_NONE, "launcher repeat rejected");
    key.flags = 0U; key.value1 = BORING_KEY_UP_VALUE;
    check(wm_key(&wm, &key) == WM_NONE, "launcher key release ignored");
    key.value1 = BORING_KEY_DOWN_VALUE; key.modifiers |= BORING_MOD_SHIFT;
    check(wm_key(&wm, &key) == WM_NONE, "launcher extra shift rejected");
    key.modifiers = BORING_MOD_SUPER;
...
static void display_wallpaper(void) {
    ...
    {
        const struct boring_display_scanout_info dynamic_info = {
            BORING_DISPLAY_SCANOUT_VERSION, 1024U, 768U, 4096U, 3145728ULL
        };
        const size_t last = (size_t)dynamic_info.byte_size - 4U;
        output = malloc((size_t)dynamic_info.byte_size);
        check(output != NULL, "M68 dynamic wallpaper output allocation");
        check(boring_display_core_init(&core, &dynamic_info),
              "M68 dynamic wallpaper display init");
        display_managed_init(&state);
        state.manager_endpoint = 99U;
        check(display_managed_control(&state, &core, 99U, 2ULL, &present) ==
              BORING_DISPLAY_STATUS_OK, "M68 manager enables dynamic wallpaper");
        check(display_managed_compose(&state, &core, output,
                                      (size_t)dynamic_info.byte_size),
              "M68 non-800x600 wallpaper compose");
        check(output[last] != 0x28U || output[last + 1U] != 0x28U ||
              output[last + 2U] != 0x28U,
              "M68 wallpaper reaches dynamic bottom-right");
        free(output);
    }
}
