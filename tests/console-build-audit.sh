...
ELF="${ROOT}/build/user/console-smoke.elf"

make -C "${ROOT}" user-console
...
if [ "${DATA_FILESZ_DEC}" -ge "${DATA_MEMSZ_DEC}" ]; then
    echo "console data PT_LOAD does not contain real BSS: filesz=${DATA_FILESZ} memsz=${DATA_MEMSZ}" >&2
    exit 1
fi
...
if printf '%s\n' "${SECTIONS}" | grep -Eq '\.(tdata|tbss) '; then
    echo 'unexpected TLS section in console smoke executable' >&2
    exit 1
fi

if printf '%s\n' "${DYNAMIC}" | grep -Fq '(NEEDED)'; then
    echo 'console smoke has a dynamic library dependency' >&2
    exit 1
fi
...
for symbol in _start boring_main boring_getpid boring_console_write boring_console_read boring_memcpy boring_memset boring_strlen; do
...
printf '%s\n' 'Userspace serial console build audit passed.'
printf 'Console smoke size: %s bytes\n' "$(wc -c < "${ELF}")"
printf '%s\n' "${TEXT_LINE}"
printf '%s\n' "${RODATA_LINE}"
printf '%s\n' "${DATA_LINE}"
