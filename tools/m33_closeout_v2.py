...
helper_paths = {
    "tools/m33_closeout.py",
    "tools/m33_closeout_v2.py",
    ".github/workflows/m33-closeout-helper.yml",
}
hits = []
for path in sorted(Path(".").rglob("*")):
    rel = path.as_posix().lstrip("./")
    if not path.is_file() or ".git" in path.parts or "build" in path.parts or rel in helper_paths:
        continue
    try:
        data = path.read_text()
