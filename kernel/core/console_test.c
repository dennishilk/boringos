...
_Static_assert(sizeof(struct boring_console_smoke_result) == 120U,
               "console smoke result layout mismatch");
...
static struct console_test_state console_state;

static void console_fail(const char *check) __attribute__((noreturn));
static void console_fail(const char *check) {
    serial_write_string("  ");
    serial_write_string(check);
    serial_write_string(": FAIL\n");
    serial_write_string("Userspace serial console self-test FAILED: ");
    serial_write_string(check);
    serial_write_string("\n");
    x86_64_halt_forever();
}
...
static bool module_find(const struct boring_limine_module_response *response,
    ...
    if (!canonical_higher(last) || ((uint64_t)address < stats.hhdm_offset) ||
        !limine_string_equals(module->path, CONSOLE_EXPECTED_MODULE_PATH) ||
        !limine_string_equals(module->string, CONSOLE_EXPECTED_MODULE_STRING)) {
        return false;
    }
...
static bool segment_permissions_valid(const struct boring_elf_image *image) {
    static const uint64_t expected_vas[CONSOLE_LOAD_EXPECTED] = {
        BORING_CONSOLE_SMOKE_TEXT_VA,
        BORING_CONSOLE_SMOKE_RODATA_VA,
        BORING_CONSOLE_SMOKE_DATA_VA
    };
    static const uint32_t expected_flags[CONSOLE_LOAD_EXPECTED] = {
    ...
    if ((image == NULL) || (image->process == NULL) ||
        (image->load_segment_count != CONSOLE_LOAD_EXPECTED)) {
        return false;
    }
...
void console_test_run(const struct boring_limine_module_response *modules) {
    ...
    serial_write_string("Userspace serial console test:\n");

    if (!module_find(modules, &module, &module_size)) {
        console_fail("boot-module-found");
    }
    serial_write_string("  boot-module-found: PASS\n");

    if (!boring_elf_validate(module, module_size,
                             (uintptr_t)BORING_CONSOLE_SMOKE_STACK_BASE,
                             (size_t)BORING_ELF_PAGE_SIZE, &validation) ||
        (validation.entry != (uintptr_t)BORING_CONSOLE_SMOKE_TEXT_VA) ||
        (validation.load_segment_count != CONSOLE_LOAD_EXPECTED)) {
        console_fail("elf64-console-image");
    }
    serial_write_string("  elf64-console-image: PASS\n");

    if (!x86_64_enable_nx() || !x86_64_nx_enabled()) {
        console_fail("nx-enabled");
    }
    serial_write_string("  nx-enabled: PASS\n");

    if (!process_init() || !syscall_init() ||
        !pmm_get_stats(&console_state.pmm_before) ||
        !process_create(&process) || (process == NULL) ||
        (process->pid != 1ULL)) {
        console_fail("process-created");
    }
    console_state.process = process;
    serial_write_string("  process-created: PASS\n");

    if (!boring_elf_load(process, module, module_size,
                         (uintptr_t)BORING_CONSOLE_SMOKE_STACK_BASE,
                         (size_t)BORING_ELF_PAGE_SIZE, &console_state.image)) {
        console_fail("load-console-image");
    }
    serial_write_string("  load-console-image: PASS\n");

    if (!segment_permissions_valid(&console_state.image)) {
        console_fail("segment-permissions");
    }
    serial_write_string("  segment-permissions: PASS\n");

    if (!ring3_user_query_mapping(&process->address_space,
                                  console_state.image.entry, &entry_info) ||
        !entry_info.executable || entry_info.writable) {
        console_fail("entry-executable");
    }
    serial_write_string("  entry-executable: PASS\n");

    if (!read_user_bytes(process, (uintptr_t)BORING_CONSOLE_SMOKE_BSS_VA,
                         &bss_initial, sizeof(bss_initial)) ||
        (bss_initial != 0ULL)) {
        console_fail("bss-loader-zeroed");
    }
    ...
    if (!ring3_shared_higher_half_supervisor_only(&process->address_space)) {
        console_fail("higher-half-supervisor-only");
    }
    serial_write_string("  higher-half-supervisor-only: PASS\n");

    serial_write_string("Console process PID: ");
    serial_write_u64(process->pid);
    serial_write_string("\nConsole user stack top: ");
    serial_write_hex_u64((uint64_t)console_state.image.stack_top);
...
void console_test_handle_exception(const struct x86_64_trap_frame *frame) {
    struct boring_console_smoke_result result;
    ...
    const uint64_t expected_pid =
        (console_state.process != NULL) ? console_state.process->pid : UINT64_MAX;

    if (!console_state.armed || (frame == NULL) ||
        (console_state.process == NULL)) {
        console_fail("exception-frame");
    }
    console_state.armed = false;

    if (!read_user_bytes(console_state.process,
                         (uintptr_t)BORING_CONSOLE_SMOKE_DATA_VA,
                         &result, sizeof(result)) ||
        !read_user_bytes(console_state.process,
                         (uintptr_t)BORING_CONSOLE_SMOKE_BSS_VA,
                         &bss_value, sizeof(bss_value)) ||
        !read_user_bytes(console_state.process, (uintptr_t)frame->rip,
                         &fault_bytes[0], sizeof(fault_bytes))) {
        console_fail("result-read");
    }

    serial_write_string("Userspace serial console:\n");

    if ((result.cs != (uint64_t)X86_64_GDT_USER_CODE_SELECTOR) ||
        ((result.cs & 0x3ULL) != 0x3ULL) || (result.entered != 1ULL)) {
        console_fail("c-entry");
    }
    serial_write_string("  c-entry: PASS\n");

    if (result.data_ok != 1ULL) {
        console_fail("initialized-data");
    }
    serial_write_string("  initialized-data: PASS\n");

    if ((result.bss_zero != 1ULL) ||
        (bss_value != BORING_CONSOLE_SMOKE_BSS_MARKER)) {
        console_fail("bss");
    }
    serial_write_string("  bss: PASS\n");
    ...
    if ((result.write_result !=
         (uint64_t)BORING_CONSOLE_SMOKE_MESSAGE_LENGTH) ||
        (result.write_ok != 1ULL)) {
        console_fail("console-write");
    }
    serial_write_string("  console-write: PASS\n");

    if ((result.read_result != 1ULL) ||
        (result.read_value != (uint64_t)BORING_CONSOLE_SMOKE_INPUT_BYTE) ||
        (result.read_ok != 1ULL)) {
        console_fail("console-read");
    }
    serial_write_string("  console-read: PASS\n");

    if ((result.echo_result != (uint64_t)BORING_CONSOLE_SMOKE_ECHO_LENGTH) ||
        (result.echo_ok != 1ULL)) {
        console_fail("console-echo");
    }
    serial_write_string("  console-echo: PASS\n");

    if ((result.sysret_resume != 1ULL) ||
        !syscall_get_stats(&syscall_stats) ||
        (syscall_stats.dispatch_count != CONSOLE_EXPECTED_SYSCALLS)) {
        console_fail("sysret-resume");
    }
    serial_write_string("  sysret-resume: PASS\n");

    if ((result.ready_to_return != 1ULL) ||
        (frame->r12 != (uint64_t)BORING_CONSOLE_SMOKE_MAIN_RETURN)) {
        console_fail("boring-main-return");
    }
    serial_write_string("  boring-main-return: PASS\n");

    if ((frame->vector != CONSOLE_GP_VECTOR) ||
    ...
    if (!descriptor_rsp0_stack_contains(handler_rsp) ||
        ...
        (frame->rsp != (uint64_t)console_state.image.stack_top)) {
        console_fail("final-tss-rsp0");
    }
    ...
    serial_write_string("\nConsole syscall dispatches: ");
    serial_write_u64(syscall_stats.dispatch_count);
    serial_write_string("\n");

    bootstrap = process_bootstrap();
    if ((bootstrap == NULL) || !process_activate(bootstrap) ||
        !boring_elf_unload(&console_state.image) ||
        !process_mark_finished(console_state.process) ||
        !process_destroy(console_state.process) ||
        !pmm_get_stats(&pmm_after) ||
        (pmm_after.free_frames != console_state.pmm_before.free_frames)) {
