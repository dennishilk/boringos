...
#define DISPLAY_TEST_PROCESS_COUNT 3U
#define DISPLAY_TEST_STACK_BASE 0x0000000040010000ULL
...
static const struct display_module_spec display_specs[DISPLAY_TEST_PROCESS_COUNT] = {
    ...
    { "/boot/user/display-client-a.elf", "boringos-display-client-a", "display-client-a" },
    { "/boot/user/display-client-b.elf", "boringos-display-client-b", "display-client-b" }
};
...
static void display_test_fail(const char *check) {
    serial_write_string("M34 display acceptance FAILED: ");
...
static bool modules_find(const struct boring_limine_module_response *response) {
    bool found[DISPLAY_TEST_PROCESS_COUNT] = { false, false, false };
    size_t module_index;
...
static void user_entry(void *argument) {
    ...
    serial_write_string("display-test: enter CPL3 pid ");
...
static bool input_hardware_init(void) {
    ...
    serial_write_string("display-test: real PS/2 mouse path online\n");
    return true;
}
...
void display_test_run(const struct boring_limine_module_response *modules) {
    ...
    serial_write_string("M34 native boring-display acceptance:\n");
    ...
    serial_write_string("display-test: three distinct processes ready\n");
    task_yield();
    display_test_active = false;
    boring_ipc_syscall_use_bootstrap_stack();
    ...
    serial_write_string("display-test: IPC/input/framebuffer/M32 resources reclaimed\n");
    ...
    serial_write_string("display-test: pmm before=");
    ...
    serial_write_string("\n");
    serial_write_string("display-test: process-exit cleanup passed\n");
    serial_write_string("M34 native boring-display acceptance passed.\n");
    x86_64_halt_forever();
}
